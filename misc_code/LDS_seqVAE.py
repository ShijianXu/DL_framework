...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        h, _ = self.lstm(z)
        x_recon = torch.sigmoid(self.fc(h))
        return x_recon
...
# 模型和优化器
model = SeqVAE(input_dim, hidden_dim, latent_dim)
optimizer = optim.Adam(model.parameters(), lr=learning_rate)
loss_function = nn.MSELoss()

# 模拟数据
x_t = torch.randn(100, seq_len, input_dim)  # 时间序列观测数据
...
for epoch in range(num_epochs):
    ...
    transition_loss = loss_function(z_next, model.encoder(x_t[:, -1, :])[0])  # 使用 encoder 的 mu 作为真实值
