...
class ResnetBlock(nn.Module):
    """
    Modify the original ResnetBlock to make it compatible 
    with the time step input in the diffusion model.
    """
...
class LinearAttention(nn.Module):
    """
    Linear attention variant, time and memory requirements scale linear 
    in the sequence length, as opposed to quadratic for regular attention.
    """
    def __init__(self, dim, heads=4, dim_head=32):
        super().__init__()
        self.scale = dim_head**-0.5
...
class PreNorm(nn.Module):
    """
    To apply groupnorm before the attention layer
    """
